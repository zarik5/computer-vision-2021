...
BoatDetector::BoatDetector() {
    HOG_descriptor = cv::Ptr<cv::HOGDescriptor>(
        new cv::HOGDescriptor(cv::Size(96, 96), cv::Size(16, 16), cv::Size(8, 8), cv::Size(8, 8), 9,
...
void BoatDetector::load(cv::String HOG_xml_file) {
    HOG_descriptor = cv::Ptr<cv::HOGDescriptor>(new cv::HOGDescriptor(
        cv::Size(96, 96), cv::Size(16, 16), cv::Size(8, 8), cv::Size(8, 8), 9));
...
void BoatDetector::train(cv::String images_folder, cv::String ground_truth_folder) {
    ...
    loadImages(images_folder, ground_truth_folder, training_images, positive_patchs);

    cv::Mat training_HOGs;
    positiveExtraction(training_images, positive_patchs, training_HOGs);
    ...
    for (size_t i = 0; i < MAX_HNM_ITERATIONS; i++) {
        ...
        for (size_t j = 0; j < training_images.size(); j++) {
            ...
            mineNegatives(training_images[j], positive_patchs[j], ships_found, training_HOGs);
            training_labels.insert(training_labels.end(), training_HOGs.rows - training_labels.size(), -1);

            std::cout << std::to_string(j) << "   ";
        }

        save(".\\HOG_Step_" + std::to_string(i) + ".xml");
    }
}
...
void BoatDetector::positiveExtraction(std::vector<cv::Mat> training_images, std::vector<std::vector<cv::Rect>> positive_patchs, cv::Mat &training_HOGs) {
    for (size_t i = 0; i < training_images.size(); i++) {
        cv::Mat padded_image;
        cv::copyMakeBorder(training_images[i], padded_image, training_images[i].cols / 2,
                           training_images[i].cols / 2, training_images[i].rows / 2,
                           training_images[i].rows / 2, cv::BORDER_CONSTANT);
        for (cv::Rect label : positive_patchs[i]) {

            int max = MAX(label.width, label.height);
            cv::Rect padded_label(training_images[i].rows / 2 + label.x + (label.width - max) / 2,
                                  training_images[i].cols / 2 + label.y + (label.height - max) / 2,
                                  max, max);
...
void BoatDetector::randomNegatives(std::vector<cv::Mat> training_images, std::vector<std::vector<cv::Rect>> positive_rects, cv::Mat &training_HOGs) {
    ...
    for (size_t i = 0; i < training_images.size(); i++) {
        ...
        while (j < MAX_NEGATIVES & tries < MAX_TRIES) {
...
void BoatDetector::mineNegatives(cv::Mat training_image, std::vector<cv::Rect> gound_truths, std::vector<cv::Rect> found_boats, cv::Mat &training_HOGs) {
